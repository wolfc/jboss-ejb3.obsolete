...
/*   protected void findPartitionName()
 ...
 //     Clustered clustered = (Clustered) resolveAnnotation(Clustered.class);
      JBossSessionBeanMetaData enterpriseBean = (JBossSessionBeanMetaData)this.getXml();
      if (enterpriseBean != null)
      {
         ClusterConfigMetaData config = enterpriseBean.getClusterConfig();
        
         if (!enterpriseBean.isClustered() || config == null)
         {
            partitionName = null;
            super.findPartitionName();
            return;
         }
         ...
         try
         {
            String replacedValue = StringPropertyReplacer.replaceProperties(partitionName);
            if (partitionName != replacedValue)
            {            
               log.debug("Replacing @Clustered partition attribute " + partitionName + " with " + replacedValue);
               partitionName = replacedValue;
            }
         }
         catch (Exception e)
         {
            log.warn("Unable to replace @Clustered partition attribute " + partitionName + 
